...
function renderCoreRosterManager() {
  ...
  // 合并只是额外的分类视图；原始账号列表始终完整保留，便于继续编辑角色。
  const visibleAccounts = accounts.map((account, index) => ({ account, index }));
  ...
  mergeGroups.forEach((group, groupIndex) => {
    ...
    memberWrap.innerHTML = (group.accountNames || []).map(name => {
      const accountIndex = accounts.findIndex(account => account.name === name);
      return accountIndex >= 0 ? renderAccountCard(accounts[accountIndex], accountIndex, 'account-merge-child') : '';
    }).join('') || '<p class="account-merge-empty">请在上方勾选属于这个大账号的区服账号。</p>';
  });
}
